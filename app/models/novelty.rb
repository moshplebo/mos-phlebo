...
class Novelty < ApplicationRecord
  ...
  def frontend
    prev_page = ''
    unless Novelty.first.published_at == published_at
      prev_page = Novelty.where('published_at > ?', published_at).last.page.material_path
    end
    next_page = ''
    unless Novelty.last.published_at == published_at
      next_page = Novelty.where('published_at < ?', published_at).first.page.material_path
    end
    ...
    data[:novelties_popular] = Novelty.all.shuffle.first(3).map do |nov|
      descr = ''
      nov.components.where(kind: 0).pluck(:metadata).map { |c| descr += "#{c['content']}. " }
      descr = ActionView::Base.full_sanitizer.sanitize(descr)
      {
          name: nov.name,
          descr: descr,
          href: nov.page.material_path
      }
    end
  ...
  def admin_frontend
    ...
    data[:id] = id
    data[:name] = name
    data[:show_on_main] = show_on_main
    data[:published_at] = Frontend.format_time(published_at)
    data[:control_url] = "/admin/novelties/#{id}"
  ...
  def set_page
    page = Page.where(main_object_type: 'Novelty', main_object_id: id).first_or_initialize
    page.slug = slug
  ...
  def news_root_page
    news_root = Page.where(slug: 'novelties', parent_id: Page.first.id).first_or_initialize

    return news_root unless news_root.new_record?

    news_root.active = true
    news_root.name = 'Новости'
    news_root.title = 'Новости'
    news_root.h1 = 'Новости'
    news_root.kind = 'Collection'

    news_root.save!

    unless Collection.find_by(collectionable_type: 'Novelty')
      Collection.create!(slug: 'novelties', page: news_root, collectionable_type: 'Novelty', name: 'Новости')
    end
